...
import { generatePrompt } from '../_lib/prompts/templates';
...
export class DocumentService {
  ...
  async generateDocument(params: {
    ...
    // 1. Overview Section
    const overviewPrompt = generatePrompt('overview', {
      githubData,
      jiraIssues,
    });
    const overviewContent = await this.llmService.generateText({
      ...
      systemPrompt:
        'You are a technical documentation expert. Generate clear, concise, and professional documentation.',
    });
    sections.push({
      title: 'プロジェクト概要',
    ...
    const requirementsPrompt = generatePrompt('requirements', {
      githubData,
      jiraIssues,
    });
    const requirementsContent = await this.llmService.generateText({
      ...
      systemPrompt:
        'You are a requirements analyst. Extract and organize requirements from the provided information.',
    });
    sections.push({
      title: '要件定義',
    ...
    const architecturePrompt = generatePrompt('architecture', {
      githubData,
      jiraIssues,
    });
    const architectureContent = await this.llmService.generateText({
      ...
      systemPrompt:
        'You are a software architect. Analyze the codebase and describe the system architecture.',
    });
    sections.push({
      title: 'システムアーキテクチャ',
    ...
    // 4. Data Flow Section
    const dataflowPrompt = generatePrompt('dataflow', {
      githubData,
      jiraIssues,
    });
    const dataflowContent = await this.llmService.generateText({
      ...
      systemPrompt:
        'You are a system analyst. Describe the data flow and interactions within the system.',
    });
    sections.push({
      title: 'データフロー',
    ...
    const technicalPrompt = generatePrompt('technical', {
      githubData,
      jiraIssues,
    });
    const technicalContent = await this.llmService.generateText({
      ...
      systemPrompt:
        'You are a senior developer. Provide technical implementation details and best practices.',
    });
    sections.push({
      title: '技術詳細',
